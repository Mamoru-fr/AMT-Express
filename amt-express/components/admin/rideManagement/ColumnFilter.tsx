...
// Portal component for rendering popup outside the table container
function PopupPortal({ children, top, left }: { children: React.ReactNode; top: number; left: number }) {
    const portalRoot = typeof document !== 'undefined' ? document.body : null;
    if (!portalRoot) return null;
    
    return createPortal(
        <div style={{ position: 'fixed', top, left, zIndex: 1000, minWidth: 280, maxWidth: 340 }}>
            {children}
        </div>,
        portalRoot
    );
...
export function ColumnFilter({
    ...
    // Fermer le popup et appliquer les filtres quand on clique à l'extérieur
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (popupRef.current && !popupRef.current.contains(event.target as Node) &&
                buttonRef.current && !buttonRef.current.contains(event.target as Node)) {
                // Appliquer les filtres sélectionnés avant de fermer
    ...
    }, [isOpen, localSelected, columnKey, onFilterChange]);
    ...
    return (
        <>
            ...
            {isOpen && (
                <PopupPortal top={popupPosition.top} left={popupPosition.left}>
